...
def get_asm(app, file, language):
    if language == "C++":
        app.output(f"Passing File: {file} and language: {language} to asmExtracter.execute_C")
...
class AssemblyTracerUI:
    def __init__(self, master):
        ...
        # File Path Entry
        self.file_label = ttk.Label(master, text="Enter file path:")
        self.file_label.grid(row=0, column=0, pady=10, padx=10, sticky=tk.W)

        self.file_entry = ttk.Entry(master, width=30)
        ...
        # Language Dropdown
        self.language_label = ttk.Label(master, text="Choose Language:")
        self.language_label.grid(row=1, column=0, pady=10, padx=10, sticky=tk.W)

        languages = ["RUST", "C++"]
        self.language_var = tk.StringVar(value=languages[0])
        self.language_dropdown = ttk.Combobox(master, textvariable=self.language_var, values=languages)
        self.language_dropdown.grid(row=1, column=1, pady=10, padx=10, sticky=tk.W)

        # Trace Assembly Button
        self.trace_button = ttk.Button(master, text="Trace Assembly", command=self.trace_assembly)
        ...
        # Footer
        self.footer_label = ttk.Label(master, text="x86 Assembly Tracer | GNU GPL v3 | https://skushagra.com", font=("Helvetica", 8))
